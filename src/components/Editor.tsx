...
import { parsePasteToGrid, sanitizeInputText, smartCleanupMarkdown } from '../utils/markdownFormatter';
...
export const Editor: React.FC<EditorProps> = ({
  ...
  const handleSmartCleanupCell = (rowIndex: number, colIndex: number) => {
    ...
    if (report.hasChanges) {
      ...
      showCleanupNotification(`Cleaned ${getCellLabel(rowIndex, colIndex)}: Fixed ${report.fixesCount} syntax item(s)`);
    } else {
      showCleanupNotification(`${getCellLabel(rowIndex, colIndex)} is already clean and standardized`);
    }
  };
  ...
  const handleSmartCleanupAll = () => {
    ...
    if (anyChanges) {
      ...
      showCleanupNotification(`Smart Cleanup: Standardized quotes, spaces & syntax (${totalFixes} fixes)`);
    } else {
    ...
    <div className={`flex flex-col h-full overflow-hidden transition-colors ${
      ...
      <div className={`h-12 px-4 border-b flex items-center justify-between gap-3 text-xs shrink-0 transition-colors ${
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {totalStats.totalChars.toLocaleString()} <span className="font-sans font-normal text-[10px] text-slate-400 ml-0.5">chars</span>
          </span>
          ...
          {totalStats.totalWarnings > 0 && (
            ...
            >
              ...
              <span>{totalStats.totalWarnings} {totalStats.totalWarnings === 1 ? 'Warning' : 'Warnings'}</span>
            </span>
          )}
        ...
        {/* Layout Selectors, Smart Cleanup & Column/Row modifiers */}
        <div className="flex items-center gap-2">
          {/* Smart Cleanup All Button */}
          <button
          ...
          <div className={`flex items-center rounded-md p-0.5 border ${
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded flex items-center gap-1.5 text-[11px] transition cursor-pointer ${
                numRows === 1 && numCols === 1
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className={`px-2 py-1 rounded-md border flex items-center gap-1 text-[11px] transition cursor-pointer ${
                isDark
      ...
      {cleanupNotification && (
        <div className={`px-4 py-2 text-xs border-b flex items-center gap-2 animate-in fade-in duration-200 ${
          isDark 
